...
class DataPreprocessor:
    ...
    def __init__(self, src_path, output_dir_processed, output_dir_reports,
        ...
        self.X_test_transformed_df = None
    ...
    def ejecutar_oversampling(self, random_state=42):
        ...
        self.metodo_uo = SMOTETomek(sampling_strategy=0.1,random_state=random_state)
        self.X_train_resampled, self.y_train_resampled = self.metodo_uo.fit_resample(
            self.X_train_transformed_df, self.y_train
        )

        self.X_train_final = self.X_train_resampled
    ...
    def ejecutar_preprocesamiento_completo(self):
        ...
        self.transformar_variables()

        # 6. Oversampling
        self.ejecutar_oversampling(random_state=42)
    ...
    def _transform_with_cols(self, X_df, cat_cols, transformer):
        ...
        if not cols_in:
            return pd.DataFrame(
                index=X_df.index,
                columns=transformer.get_feature_names_out(cols_in)
            ).fillna(0).astype("int8")

        # Transformar datos
        X_cat = pd.DataFrame(
            transformer.transform(X_df[cols_in]),
            columns=transformer.get_feature_names_out(cols_in),
            index=X_df.index
        )
